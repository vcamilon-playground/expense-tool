...
> **This catalogue is a point-in-time snapshot for human readers.** The spec files in `apps/e2e/tests/` are the source of truth — when they and this list disagree, trust the specs. **How** tests are written (Page Object Model, selectors, coverage, cleanup, cross-viewport) lives in [`TEST_AUTOMATION_STANDARDS.md`](TEST_AUTOMATION_STANDARDS.md); the `e2e-author` agent (`.claude/agents/e2e-author.md`) follows it.
