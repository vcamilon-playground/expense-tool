...
- Every money movement is recorded in `income_transactions`: `deduct` (expense/recurring payment), `add` (Add Money), `transfer` (one row, From → To), and `edit` (only when a source's balance changes — name/brand-only edits are not logged). Logging via `logIncomeTransaction` is **best-effort** (write errors are swallowed to console; the balance update is authoritative). Deleting an income source **retains** its history (`source_id` → null, `source_label`/`counterparty_label` snapshots persist). `archiveOldIncomeTransactions(userId)` lazily flips rows older than 3 months (`ARCHIVE_AFTER_MONTHS`) to `archived=true` on Income-page load (no cron); the `/income` "Transaction History" section hides archived rows behind a "Show archived" toggle and masks amounts under the existing privacy eye. Pure helpers live in `lib/income-history.ts`; `listIncomeTransactions(userId, includeArchived=false)` reads them.
