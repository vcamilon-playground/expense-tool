...
| `recurring_expenses` | `id`, `user_id`, `name`, `amount`, `category_id`, `cadence` (weekly/monthly/yearly), `next_charge_date`, `active`, `is_variable` (bool, default false) |
| `income_sources` | `id`, `user_id`, `type` (bank/ewallet/cash), `name` (null for cash), `balance` |
| `income_transactions` | `id`, `user_id`, `source_id` (FK, **on delete set null**), `source_label` (snapshot), `counterparty_id`/`counterparty_label` (transfers), `kind` (deduct/add/transfer/edit), `amount`, `balance_before`, `balance_after`, `note`, `archived`, `created_at` |
| `reminders` | `id`, `user_id`, `title`, `remind_date`, `cadence` (once/weekly/monthly/yearly), `active` |
| `maya_savings` | `id`, `user_id` (**unique** — one row/user), `done_weeks` (`integer[]` of completed week numbers for the Maya weekly savings tracker) |
| `investments` | `id`, `user_id`, `name`, `type` (fund/stocks/crypto/savings/retirement/other), `platform`, `principal` (cost basis), `current_value` (updated manually), `active` |
| `debts` | `id`, `user_id`, `name`, `kind` (credit_card/loan/personal/installment/other), `lender`, `principal` (original), `balance` (outstanding), `monthly_payment`, `interest_rate`, `due_day` (1-31), `active`, `last_paid_month` (`'YYYY-MM'` — pure "paid this month" status flag, auto-resets on month rollover; never affects balance/net worth) |
| `income_snapshots` | `id`, `user_id`, `week_ending` (the Sunday ending a week), `total` (grand total of all income source balances at that point), `unique(user_id, week_ending)` — one row per user per week |
...
- A recurring expense with `is_variable=true` has an **optional** estimate `amount` (blank stored as 0); fixed ones keep a required `amount > 0`. Both pay modals (Confirm Payment / Pay Now) expose a required, editable "Amount paid" field (prefilled with the stored amount when > 0), and the recorded expense **and** any income deduction use the **entered** amount. The recurring table, dashboard "Upcoming Charges", and the `/notifications` due-body are variable-aware (they say the amount varies). `recurringAmountLabel(r)` in `@expense/shared` renders the display label (fixed → `formatMoney`; variable with estimate → `~<money>`; variable with none → `Varies`).
- `income_sources.type` is `'bank'`, `'ewallet'`, or `'cash'`. Only one `cash` entry is allowed per user (enforced in app code). When an expense is **created** (not edited), the user may optionally select an income source to deduct from; `deductFromIncomeSource` is called immediately after `createExpense`. No restoration occurs on edit or delete. `addToIncomeSource(id, amount)` tops up a single source's balance (the "Add Money" action on the Income page). `transferIncome(fromId, toId, amount)` moves money between two sources (deduct + add) — the "⇄ Transfer" action lets the user pick both ends; "🏧 Withdraw" (shown only when a cash source **and** at least one non-cash source exist) is a thin UI convenience over the same `transferIncome` call with the destination fixed to the user's single Cash on Hand source — no new DB function or schema.
- Every money movement is recorded in `income_transactions`: `deduct` (expense/recurring payment), `add` (Add Money), `transfer` (one row, From → To), and `edit` (only when a source's balance changes — name/brand-only edits are not logged). Logging via `logIncomeTransaction` is **best-effort** (write errors are swallowed to console; the balance update is authoritative). Deleting an income source **retains** its history (`source_id` → null, `source_label`/`counterparty_label` snapshots persist). History lives on its **own page** at `/income/history` (reached via the "Transaction History" button to the right of Transfer on `/income`); rows are **grouped by month**, hide archived rows behind a "Show archived" toggle, and mask amounts under the same privacy eye (localStorage `income-amounts-visible`). Retention is enforced lazily on load (no cron): `archiveOldIncomeTransactions(userId)` flips rows older than 3 months (`ARCHIVE_AFTER_MONTHS`) to `archived=true`, and `deleteOldIncomeTransactions(userId)` permanently deletes rows older than 6 months (`DELETE_AFTER_MONTHS`; also fired best-effort on `/income` load). Pure helpers (`groupTransactionsByMonth`, `archiveCutoff`, `deleteCutoff`, …) live in `lib/income-history.ts`; `listIncomeTransactions(userId, includeArchived=false)` reads them.
- The **Maya Weekly Savings** tracker lives at `/income/maya` (reached via the "💜 Maya Savings" ghost link in the Income header, between "⇄ Transfer" and "🧾 Transaction History"). It is a **tracker only — no money movement**. The plan is deterministic: ₱100 on the first Friday (`MAYA_START_*` = 2026-01-09), +₱`MAYA_WEEKLY_INCREMENT` (100) each Friday through the last Friday of `MAYA_END_YEAR` (2026) — 51 weeks, ₱132,600 goal. Which weeks are transferred is a manual checklist persisted **per-user in the `maya_savings` table (`done_weeks integer[]`) so it syncs across devices**. On load `getMayaSavings` reads the row; if none exists `ensureMayaSavings` seeds every Friday before today as done and inserts it (idempotent — a racing/StrictMode duplicate insert hits the unique constraint and re-reads the winner). Toggling a week does an optimistic UI update, then persists the full sorted/deduped array via `setMayaSavingsWeeks` through a **serialized write chain** so racing toggles can't clobber each other; a save failure shows a `.field-error` banner and resyncs the UI to the DB, and a load failure shows a banner-only view. The page wires `refreshKey` to refetch on resume. Pure logic (`mayaSchedule`, `mayaGoal`, `mayaTotals`, `initialDoneWeeks`, `currentMayaWeek`, `formatMayaDate`) lives in `lib/maya-savings.ts` and is unit-tested.
- The dashboard shows a **"Weekly Income Trend — Past 6 Weeks"** card charting the grand total of all income source balances at each of the last 6 completed weeks (Sunday cutoffs), from `income_snapshots`. Snapshots are captured **lazily on dashboard load** (going forward, no cron): each load best-effort `upsertIncomeSnapshot`s the CURRENT week's row = the live grand total (the in-progress week is captured but not charted). Empty state until weeks accrue. Pure helpers (`currentWeekSunday`, `incomeWeeklyTrend`) live in `lib/income-trend.ts` and are unit-tested; the chart is `components/IncomeTrendChart.tsx` (mirrors `LineTrendChart`).
- The **Portfolio** page at `/portfolio` (sidebar nav between Income and Expenses; also reachable from the "📊 Portfolio" ghost link in the Income header, which is how it is reached on mobile since the bottom tab bar is full) tracks `investments` and `debts` side by side. It is **track-only — no money movement**: nothing writes to `income_sources` and no expense or `income_transactions` row is ever created. `current_value` (investments) and `balance` (debts) are typed in by the user, since there is no market-data API on the free tier. The page **reads** `listIncomeSources` purely to compute **Net Worth = cash + investment current value − debt balance**, and shares the Income page's privacy eye (`localStorage` key `income-amounts-visible`) so the mask stays in sync. Debts render as cards with a payoff progress bar (`debtPaidPercent`, sorted by `due_day` via `sortDebtsByDueDay`); investments render as a table with computed gain/loss. Each debt with a monthly payment also shows **derived installment months-remaining** ("· {remaining} of {total} month(s) left", from `debtInstallments`) and a **net-worth-neutral "Mark paid this month" status**: a "Mark paid" button stamps `debts.last_paid_month` (via `updateDebt`) to the current month → shows a "✓ Paid this month" pill + "Undo"; it only sets/clears `last_paid_month` (never the balance), auto-resets on month rollover, and a muted Debts-header note states the payment is your separately-recorded expense so Net Worth is unchanged. All maths is pure in `lib/portfolio.ts` (`investmentGain`, `investmentTotals`, `debtPaidPercent`, `debtPaidAmount`, `debtTotals`, `netWorth`, `sortDebtsByDueDay`, `ordinalDay`, `debtInstallments`, `currentYearMonth`, `isDebtPaidThisMonth`) and unit-tested — nothing derived is ever stored.
