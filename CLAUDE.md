...
- The **Maya Weekly Savings** tracker lives at `/income/maya` (reached via the "💜 Transfer to Maya Savings" ghost link in the Income header, between "⇄ Transfer" and "🧾 Transaction History"). It is a **tracker only — no database table, no money movement**. The plan is deterministic: ₱100 on the first Friday (`MAYA_START_*` = 2026-01-09), +₱`MAYA_WEEKLY_INCREMENT` (100) each Friday through the last Friday of `MAYA_END_YEAR` (2026) — 51 weeks, ₱132,600 goal. Which weeks are transferred is a manual checklist persisted in `localStorage` under `maya-savings-done` (`LS_DONE_KEY`, a JSON array of week numbers); on first visit it seeds every Friday before today as done, then is purely manual. Pure logic (`mayaSchedule`, `mayaGoal`, `mayaTotals`, `initialDoneWeeks`, `currentMayaWeek`, `formatMayaDate`) lives in `lib/maya-savings.ts` and is unit-tested.
